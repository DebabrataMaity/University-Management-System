...
@login_required()
def placement(request, class_id):
    return render(request,'info/placement.html')

@login_required()
def notification(request, class_id):
    allPosts=Notify.objects.all()
    context={'allPosts':allPosts}
    return render(request,'info/notification.html',context)

@login_required()
def notifyPost(request,slug):
    post=Notify.objects.filter(slug=slug).first()
    print(post)
    context={'post':post}
    return render(request,'info/notifyPost.html',context)
...
def t_report(request, assign_id):
    ...
    for stud in ass.class_id.student_set.all():
        a = StudentCourse.objects.get(student=stud, course=ass.course)
        b = AttendanceTotal.objects.get(student=stud, course=ass.course)
        c=StudentCourse.objects.get(student=stud, course=ass.course)
        #print(b.attendance)
        #print(c.get_cie())
        sc_list.append(a)
        #atte.append(b)
        ...
        if b.attendance < 20:
            atte[0]=atte[0]+1
        elif b.attendance > 20 and b.attendance <40:
            atte[1]=atte[1]+1
        elif b.attendance > 40 and b.attendance <60:
            atte[2]=atte[2]+1
        elif b.attendance > 60 and b.attendance <80:
            atte[3]=atte[3]+1
        else:
            atte[4]=atte[4]+1 
        ...
        if c.get_cie() < 20:
            ci[0]=ci[0]+1
        elif c.get_cie() > 20 and c.get_cie() <40:
            ci[1]=ci[1]+1
        elif c.get_cie() > 40 and c.get_cie() <60:
            ci[2]=ci[2]+1
        elif c.get_cie() > 60 and c.get_cie() <80:
            ci[3]=ci[3]+1
        else:
            ci[4]=ci[4]+1     
    return render(request, 'info/t_report.html', {'sc_list': sc_list,
...
def t_notify(request):
    if request.method == 'POST':
            if request.POST.get('title') and request.POST.get('content'):
                post=Notify()
                post.sno= request.POST.get('sno')
                post.author= request.POST.get('author')
                post.title= request.POST.get('title')
                post.content= request.POST.get('content')
                post.slug= request.POST.get('slug')
                post.timestamp= request.POST.get('timestamp')

                post.save()

            return render(request, 'info/t_notify.html')
    return render(request, 'info/t_notify.html')  
